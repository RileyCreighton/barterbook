...
**Current deployment — longer-lived signing:** The owner’s version 8 test saved Bob’s signature, then Alice’s verified signature arrived after the chain’s roughly 25-second signing lifetime. [Measured timing](evidence/hosted/browser-v8-signing-expiry.json) identifies expiry as this retry’s failure. The owner requested [longer-lived signing](docs/durable-signing.md), including wallet-controlled setup, on-chain cancellation and recovery. [Updated two-person and three-person steps](docs/browser-transaction-retest.md). [Build `c9e9ee0` is deployed and verified](evidence/hosted/durable-signing-deployment-c9e9ee0.json); [292 local tests passed](evidence/local/durable-signing-tests.json). [CI also passed all 292 tests](evidence/local/ci-36174285244.json). Actual Solflare setup and finalized browser exchanges remain unverified.
