...
**Live Devnet-configured prototype:** [open the no-wallet guide](https://barterbook-devnet.rileycreighton.workers.dev/#/walkthrough), [source](https://github.com/RileyCreighton/barterbook), [passing CI for deployed commit `b02e90b`](https://github.com/RileyCreighton/barterbook/actions/runs/36109186427).

The hosted website, API and D1 authentication/persistence checks pass. Cloudflare access is restored; the deployed app has three validated mock mints, six demo participants and the SDK proof view. All three browser participants' authentication records were independently found in D1; the user reports successful Phantom logins and portfolios showing 1,000 of each mock token. **Live settlement is disabled:** browser transaction approvals and hosted financial-path CPU checks remain pending. Alice created the correct 10 TEST-A + 5 TEST-B / 20 TEST-C offer with Bob as fee payer; acceptance/readiness remains pending. Developer-run SDK tests separately finalized a real devnet basket, three-owner exchange and controlled failure with zero token settlement. [Their receipts](evidence/devnet/20260925-sdk-proof-01/) are distinct from browser settlement and local LiteSVM evidence. Mock assets never represent PRE holdings; mainnet is disabled.

The current deployment is commit `b02e90b`, Worker version `69e2cdac-ab4d-423b-a424-837d799b1066` (September 25, about 07:56 UTC); [16 hosted HTTP checks passed](evidence/hosted/http-smoke-1790323034655.json). Wallet account-list handling, stale-cookie identity protection, mobile overflow and dated expiry fixes passed their local checks but are not deployed yet. Their combined verification/CI is still pending.
...
The target recurring cost is **$0 at demonstration traffic levels**. [Current published allowances](docs/release/provider-limits.md) include Workers Free's 100,000 dynamic requests/day and 10 ms CPU/request, D1's 5 million rows read and 100,000 written/day, and Helius Free's 1 million credits/month. The owner confirmed Workers Free and dashboard quotas, and reports `GET /holdings` CPU of 2 ms. That dashboard observation has no automated raw telemetry capture and does not establish financial-path CPU headroom; telemetry API access still returns 403. Hosted financial verification remains unmeasured; no paid upgrade is authorized.
