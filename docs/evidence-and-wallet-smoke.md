...
| Hosted API SDK two-for-one basket | 2026-09-25 08:43:23 | Public devnet through live Worker/D1; SDK signers | [Receipt](../evidence/devnet/20260925-sdk-proof-01/hosted-hosted-financial-02-basket-receipt.json) | FINALIZED, slot 503906415; 10,000 lamports fee; preserved message; public application history. Not browser proof. |
| Hosted API SDK three-way exchange | 2026-09-25 08:43:40 | Public devnet through live Worker/D1; SDK signers | [Receipt](../evidence/devnet/20260925-sdk-proof-01/hosted-hosted-financial-02-ring-receipt.json) | FINALIZED, slot 503906516; 15,000 lamports fee; preserved message; public application history. Not browser proof. |
...
### Historical deployed rehearsal — `f71b03d`

Current deployment is commit `f71b03decc624965ed74d0d4ff9b8844cb350d5d`, Worker version `0714cb11-6087-44db-abaa-c4127f72f61b`. Settlement is enabled for the authorized devnet rehearsal. Both history-trust flags were enabled following the archived [read-only provider assessment](../evidence/devnet/20260925-sdk-proof-01/recovery-provider-checks/); each recovery still needs its original transaction's evidence. Wallet/session/mobile/expiry fixes are deployed, and [CI for `46c9` passed](https://github.com/RileyCreighton/barterbook/actions/runs/36111481548). Earlier disabled-settlement and pending-deployment checkpoints above remain historical.

The owner reports Alice and Bob accepted and marked ready in their browser room beginning `4cff89`; this is an owner report, not browser transaction-signature proof. In the separate hosted SDK rehearsal, the first signature persisted before HTTP 409 because the D1 result counted two changes including a trigger. The compare-and-swap result check is fixed in `f71`. Original expired attempt `c4bba8a0-95c1-4930-a493-53965a7bbb8e` remains `STATUS_UNKNOWN`, with no submission-started marker, and recovery is under diagnosis. Preserve all original bytes, signatures, locks and journals; neither expiry nor this missing marker proves that a replacement is safe.

The owner has not located the financial rehearsal requests in the dashboard. `GET /holdings` CPU of 2 ms remains an owner-reported observation only, and telemetry API access remains 403. Hosted financial-path CPU and real browser transaction signing are still unproven. This checkpoint does not claim the hosted SDK rehearsal settled.

### Historical recovery and readiness checkpoint — `25cae14`

Current deployment is `25cae1414a4ce42bf2cb8347c875b311c00dd5c5`, Worker `12e768df-54cb-4a92-95d8-30435f6f2cf5`. [CI run 36113017348](https://github.com/RileyCreighton/barterbook/actions/runs/36113017348) passed 194/194 tests, zero failures; [its downloaded report](../evidence/local/ci-36113017348.json) is retained. [Remote D1 evidence](../evidence/hosted/browser-readiness-1790324985916.json) independently confirms Alice and Bob both accepted version 1 and marked ready: room state READY, current attempt null. No browser transaction signing is claimed.

[Hosted original-attempt diagnosis](../evidence/devnet/20260925-sdk-proof-01/hosted-original-reconciliation-1790324886082.json) reports primary Helius healthy/trusted, blockhash invalid and original status/transaction absent after expiry. The fallback `api.devnet.solana.com` returned HTTP 403 at `getHealth`, so it could not supply authoritative absence. The attempt remains `STATUS_UNKNOWN` and unsafe to retry. This failure is provider access, not CPU or a D1 query cap. The tested dRPC public route returned HTTP 400/code 35 requiring paid access; no purchase was made. [Alchemy Free](backup-rpc.md) is the proposed independent backup, not configured or verified yet.

Actual hosted CPU was collected in [496 untruncated invocation rows](../evidence/hosted/cpu-2026-09-25T08-32-51.375Z.json). Financial samples were prepare **60 ms** (one sample, version `46c9`), signature **16 ms** (one sample, HTTP 409 after the first signature persisted), and reconciliation **15/23 ms** (two samples). Holdings peaked at 19 ms across four samples; assets at 14 ms across three. No `exceededCpu` outcome was recorded and invocation outcomes were `ok`, but these sparse measurements exceed the published **10 ms Workers Free allowance** and do not pass the budget gate. The owner's earlier 2 ms holdings observation was incomplete. No paid plan was enabled; optimization and fresh measurement are required before further wallet transaction prompts. The Alchemy URL is saved privately, with validation in progress; it is not yet configured or verified as the recovery fallback.

### Current hosted API milestone — `837b06c`

**Current hosted milestone — September 25, 08:43–08:44 UTC:** commit `837b06c735b8fad6ac6302031a6469be84be28ef`, Worker `f3221118-0847-45f2-9c36-e2799cba82b7`. Alchemy passed [11 read-only checks](../evidence/devnet/20260925-sdk-proof-01/recovery-provider-checks/alchemy-1790325426672-5f5fe58c-7548-4e4b-8ad3-ef0ec142d7a1.json) and is configured as the server-only backup. At 08:38, [original-attempt reconciliation](../evidence/devnet/20260925-sdk-proof-01/hosted-original-reconciliation-1790325517997.json) reached `EXPIRED_UNLANDED`, `safeToRetry: true`, with both providers healthy and trusted; the original journal remains preserved.
