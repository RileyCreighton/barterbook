...
| `RPC_ADDRESS_HISTORY_TRUSTED`, `FALLBACK_ADDRESS_HISTORY_TRUSTED` | Default `false`. Separate per-provider assertions of complete fee-payer address-index coverage across the frozen attempt lifetime; general history trust or a null response is insufficient. Bounded message-history recovery is deployed in `63378c6`; these flags are enabled on that deployment after assessment, and the original browser attempt has verified safe-expiry status; browser settlement remains pending. |
