...
**Bounded recovery without a recorded identifier — deployed in `63378c6`, original browser safe expiry verified:** if the fee payer never uploads its signature, the coordinator cannot derive the identifier of a transaction that might have been completed outside the application. Recovery uses a read-only lookup of the frozen message in the fee payer's finalized address history. It requests no new signatures and sends no transaction. The UI still asks the fee payer to sign first to reduce this case; deleting records or switching blockhashes is never a safe workaround.

The new `RPC_ADDRESS_HISTORY_TRUSTED` and `FALLBACK_ADDRESS_HISTORY_TRUSTED` settings default to `false`. Each is a separate operator assertion that its provider has a complete fee-payer address index covering the original lifetime, in addition to that endpoint's general history-trust flag. A null response or passing connectivity probe does not prove address-index completeness.

The bounded lookup requires all of these checks:

1. Prove the context slot is a valid lower boundary by fetching that finalized block and matching its blockhash to the frozen message's original blockhash. An unanchored context slot cannot define the scan window.
2. Obtain `absoluteSlot` and `blockHeight` together from one finalized `getEpochInfo` response. Its height must exceed the original last-valid height and its root must cover the context slot. Pass that root as `minContextSlot` to the finalized address-history queries so a stale endpoint cannot supply a scan from before expiry.
3. Read at most one 50-entry page and inspect at most two transaction candidates at or above the anchored context slot per provider. Validate unique signatures, finalized status, ordering and root bounds. The page must cross below the lower boundary or prove exhaustion; a nonempty short page requires an empty one-entry `before` continuation. A full page that does not cross the boundary, an exceeded candidate cap, missing metadata or any incomplete result retains the original locks.
4. Fetch finalized transaction bytes and metadata for every candidate. Bind identity, slot and outcome to its index entry. A discovered original must match the exact frozen message bytes, pass the strict transaction verifier and every required signature check, and preserve any stored partial signatures or complete signed bytes. Cryptographically bind unrelated candidates before excluding them.
5. Complete the scan for every configured provider, with at least two independently trusted, healthy provider hosts, before adopting a discovered identifier or declaring absence. Conflict or partial completion cannot promote an identity that would bypass the message-history scan on a later retry.

Credible exact-message success is remembered even if a subsequent metadata check, provider call or scan fails. Previously observed success and verified receipts remain sticky; later absence cannot authorize a repeat. Only complete authoritative absence after expiry, or verified finalized failure of the exact original, permits the existing replacement policy. The [stopped browser attempt has a verified safe-expiry result](../evidence/hosted/browser-original-reconciled-026ed6d7.json). The deployed repair also binds known-identifier recovery to fresh finalized context. Safe expiry does not prove browser signing or settlement.
...
Automated local tests cover financial arithmetic, strict signing, receipt truth, auth, database races and recovery. HTTP settlement integration tests mock RPC while exercising raw Token-2022 decoding and actual SQLite constraints. Separately, LiteSVM 0.8.0 executes the bundled Token-2022 and ATA programs with signature/blockhash verification: the two-for-one basket and three-owner exchange succeed with missing receiving accounts, while a deliberately failing final leg rolls back token effects and ATA creations but retains the network fee. These isolated local program results, immutable signature-round-trip hashes and exact account effects are recorded in [local execution evidence](../evidence/local/litesvm-execution.json); they are not public-network RPC receipts. A local workerd HTTP smoke passed. The earlier local warmed workerd profile measured approximately 4.74 ms sampled active CPU per strict verification; that isolated result did not establish full hosted request CPU or Free-tier headroom.

**Historical unfunded snapshot:** the initial faucet attempt did not establish funded public-network wallets. At that point, no completed public devnet barter or three-owner exchange, public-network atomic rollback, real browser-extension signature round trip, or devnet/mainnet settlement receipt was claimed. Subsequent authorized funding and devnet execution are recorded below and in the evidence ledger. PRE eligibility and mainnet compatibility remain separate gates; devnet mock assets are not real PRE holdings. See the [execution checklist](evidence-and-wallet-smoke.md), [setup guide](../README.md), and [submission draft](submission.md).

**Historical published checkpoint — 2026-09-25 06:58 UTC:** the single Worker is live at [the devnet demo](https://barterbook-devnet.rileycreighton.workers.dev), with all six remote D1 migrations applied. Initial [source commit `629ed20`](https://github.com/RileyCreighton/barterbook/commit/629ed20da12e68f80bc167cfbca87506110bdf15) passed [GitHub CI with 143 tests](https://github.com/RileyCreighton/barterbook/actions/runs/36104365100). Hosted desktop/mobile no-wallet checks passed; [16 HTTP checks](../evidence/hosted/http-smoke-1790319106255.json) separately verified endpoints and SDK authentication/session controls. The registry remains empty and settlement disabled. Hosted financial-path CPU remains unmeasured because telemetry access returned 403; these HTTP checks are not browser-wallet or settlement proof.


**Current checkpoint — 2026-09-25, recovery deployment verified 09:13 UTC:** [commit `63378c69a5c831f46058f8d5adee6dcc4bb040c9`](../evidence/hosted/recovery-deployment-63378c6.json) (Worker `82b42cd9-aa9c-4111-96e7-666060f13c2d`) has three validated devnet mock mints and six demo participants. Authorized devnet settlement is enabled. [Earlier `837b06c` CI passed 212/212 tests, with zero failures or pending tests](../evidence/local/ci-36114386113.json). The hosted API finalized a [basket](../evidence/devnet/20260925-sdk-proof-01/hosted-hosted-financial-02-basket-receipt.json) and [three-way exchange](../evidence/devnet/20260925-sdk-proof-01/hosted-hosted-financial-02-ring-receipt.json) with disposable SDK signers; both receipts are in public history. These are not browser-wallet results. The [original browser attempt safely reconciled](../evidence/hosted/browser-original-reconciled-026ed6d7.json) to `EXPIRED_UNLANDED`, `safeToRetry: true`, with both explicit address-history assertions enabled after assessment; its journal and null identifier are preserved. The owner reports same-room version 2 renewed with both participants accepted and ready; requested Bob-then-Alice transaction approvals and submission remain pending verification. No browser trade is claimed. Local recovery/settlement checks passed 58 tests plus typecheck/build; [recovery-deployment CI passed 242/242 tests, with zero failures or pending tests](../evidence/local/ci-36117159889.json). [Latest hosted recovery CPU](../evidence/hosted/cpu-2026-09-25T09-16-06.908Z.json) measured 60 ms in one successful sample, above the Workers Free 10 ms allowance. No paid upgrade was made.
