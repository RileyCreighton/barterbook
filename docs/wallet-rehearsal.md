...
For the deliberate failing-final-leg case, retain transaction-specific metadata showing all token effects and new ATA creation rolled back; the network fee may still apply. Local runtime proof already exists, but record public devnet proof separately if performed. Complete the detailed [wallet and evidence checklist](evidence-and-wallet-smoke.md).
